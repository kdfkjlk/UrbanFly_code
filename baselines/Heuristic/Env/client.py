...
drone_config = {
    ...
    'camera_names': {'external': {'downward': 'seg_cam', 'forward': 'forward_check'},
                     'internal': {'downward': 'downward_custom', 'forward':'forward_downward_custom'}},
...
class Drone_tool():
    ...
    def initialize_client(self, client_port):
        ...
        self.client = airsim.VehicleClient(ip="127.0.0.1", port=client_port)
    ...
    def set_external_camera_pose(self, obj_position, camera_height=30, help_cam=False):
        if not help_cam:
            camera_name = self.camera_name['external']['downward']
        else:
            camera_name = self.help_camera_name

        seg_cam = self.client.simGetCameraInfo(camera_name, external=True).pose
        camera_pose = airsim.Pose(airsim.Vector3r(obj_position[0], obj_position[1], -camera_height), seg_cam.orientation)
        self.client.simSetCameraPose(camera_name, camera_pose, external=True)
        # time.sleep(0.2)
        print('set camera {} at pose {}'.format(camera_name, camera_pose.position))
        return camera_pose

    def get_external_camera_pose(self, help_cam=False):
        if not help_cam:
            camera_name = self.camera_name['external']['downward']
        else:
            camera_name = self.help_camera_name
        return self.client.simGetCameraInfo(camera_name, external=True).pose


    def check_pose_collision(self, help_cam_position_xy, help_cam_height):
        camera_name = self.camera_name['external']['forward']
        help_cam_pose = airsim.Vector3r(help_cam_position_xy[0], help_cam_position_xy[1], -help_cam_height)
        ...
        def get_depth_ratio():
            ...
            image = np.clip(img1d, 0, 255)
            image = np.array(image, dtype=np.uint8)
            collision_result = (image / 255 < 0.004).sum() / image.flatten().shape[0]
        ...
        collision_flag = []
        for angle in angles:
            orientation = airsim.to_quaternion(0, 0, angle)
            camera_pose = airsim.Pose(help_cam_pose, orientation)
            self.client.simSetCameraPose(camera_name, camera_pose, external=True)
            collide = get_depth_ratio()

            if collide:
                collision_flag.append(True)

        return len(collision_flag) > 0  ## len>0 means contains collision
    ...
    def check_cam_in_building(self, help_cam_position_xy, help_cam_height, dis_thres):
        ...
        def get_depth_dis():
            image_type = airsim.ImageType.DepthPlanar
            responses = self.client.simGetImages([airsim.ImageRequest(camera_name, image_type, True, False)],external=True)
            response = responses[0]
            img1d = np.array(response.image_data_float, dtype=float)
            image = np.maximum(np.ones(img1d.size), img1d)
            image = image.reshape(response.height, response.width)
        ...
        for angle in angles:
            ...
            camera_pose = airsim.Pose(help_cam_pose,orientation)
            self.client.simSetCameraPose(camera_name, camera_pose, external=True)
            dis2near = get_depth_dis()

            if dis2near < dis_thres:
                in_building_flag.append(True)

        return len(in_building_flag) == 4  ## len=4 means in building


    def get_current_extra_depth_img(self):
        def get_image(camera_name):
            image_type = airsim.ImageType.DepthPerspective
            responses = self.client.simGetImages([airsim.ImageRequest(camera_name, image_type, True, False)], external=False)
        ...
        while time_sleep_cnt <= 20:
            try:
                # image_up, response_up = get_image(camera_name = 'extra_upward_depth_cam')
                image_down, _ = get_image(camera_name = 'extra_downward_depth_cam')
                break

            except:
                time_sleep_cnt += 1
                # image_up = image_down = None
                image_down = None
                print('Error when getting upward and downward depth images')

        # return image_up, image_down
        return image_down
    ...
    #     def extract_depth_img(response):
    #         if response.width == 0 or response.height == 0:
    #             image = np.zeros((480, 640, 3), dtype=np.uint8)  # 全黑RGB图
    #         else:
    #             img1d = np.array(response.image_data_float, dtype=float)
    ...
    #     requests = [
    #         airsim.ImageRequest(self.camera_name['internal']['forward'], airsim.ImageType.Scene, False, False),
    #         airsim.ImageRequest(self.camera_name['internal']['downward'], airsim.ImageType.Scene, False, False),
    #         airsim.ImageRequest(self.camera_name['internal']['forward'], airsim.ImageType.DepthPlanar, True, False),
    #         airsim.ImageRequest(self.camera_name['internal']['downward'], airsim.ImageType.DepthPlanar, True, False)
    #     ]

    #     time_sleep_cnt = 0
    #     while time_sleep_cnt <= 20:
    #         try:
    #             responses = self.client.simGetImages(requests, external=False)
    #             break

    #         except:
    #             time_sleep_cnt += 1
    #             print('Error when getting 4 images')
    #             time.sleep(2)

        
    #     rgb_forward = extract_rgb_img(responses[0])
    #     rgb_downward = extract_rgb_img(responses[1])
    #     depth_forwrad = extract_depth_img(responses[2])
    #     depth_downward = extract_depth_img(responses[3])

    #     return rgb_forward, rgb_downward, depth_forwrad, depth_downward
    ...
    def get_current_image(self, image_type, external, angle, help_cam=False):
        ...
        def get_image(image_type, external, camera_name):
            ...
            if image_type == 'scene':
                ...
                responses = self.client.simGetImages([airsim.ImageRequest(camera_name, image_type, False, False)], external=external)
                response = responses[0]
                img1d = np.frombuffer(response.image_data_uint8, dtype=np.uint8)
                image = img1d.reshape(response.height, response.width, 3)
    ...
    def get_drone_segment_img(self):

        def _get_seg_img(cam_name):
            image_type = airsim.ImageType.Segmentation
            responses = self.client.simGetImages([airsim.ImageRequest(cam_name, image_type, False, False)],
                                                 external=False)
            response = responses[0]
            img1d = np.frombuffer(response.image_data_uint8, dtype=np.uint8)
            image = img1d.reshape(response.height, response.width, 3)
        ...
        while time_sleep_cnt <= 20:
            try:
                image_forward = _get_seg_img('forward_downward_custom')
                image_downward = _get_seg_img('downward_custom')
                image = [image_forward, image_downward]
                break

            except:
                time_sleep_cnt += 1
                image = None
                print('Error when getting segmentation image, time_sleep_cnt: {}'.format(time_sleep_cnt))
