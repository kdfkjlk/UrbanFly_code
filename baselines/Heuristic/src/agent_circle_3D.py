...
def evaluate_with_collision(airsim_env, xy_trajectory, current_z, det_conf_thres, img_height):
    ...
    for step, tra in enumerate(xy_trajectory):
        ...
        # Try to place drone at target position, ascend if collision occurs
        max_ascend_attempts = 5  # Maximum number of 5m ascends
        ascend_step = 5.0  # 5 meters ascend step
        
        for ascend_attempt in range(max_ascend_attempts + 1):
            # Place drone at target position
            target_pose = tra + [current_z]
            airsim_env.drone_tool.reset_drone_pose(target_pose, init_yaw=target_yaw)
            
            # Check for collision after placing drone
            collision_detected = airsim_env.check_collision()
            
            if not collision_detected:
                break
            else:
                # Collision detected, try ascending
                if ascend_attempt < max_ascend_attempts:
                    current_z += ascend_step
                else:
                    print(f"Max ascend attempts reached, staying at height {current_z}m")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--flytype", type=str, default="zigzag", choices=["spiral", "zigzag"])
    ...
    for idx, episode_info in enumerate(data):
        ...
        if args.output_dir is None:
            output_dir = os.path.join(BASELINE_DIR, 'logs', 'agent_3D')
        else:
...
#     z_height = 20.0
#     R = 30.0
#     path = generate_zigzag_center_start(z_height, R)
#     plot_zigzag_with_arrows(path, R, arrow_interval=1)
#     plt.show()
