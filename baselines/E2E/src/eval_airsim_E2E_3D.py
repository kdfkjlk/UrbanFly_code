...
import time
import json
from Components.evaluation_env import DummyEnv_obstacle_airsimmode_evaluate, DummyEnv_obstacle_airsimmode_evaluate_fly3D
...
def parse_args():
    ...
    parser.add_argument("--client_port", default=41451, type=int, help="Client_port for AirSim map connection")
    parser.add_argument("--env_mode", default="dummy_fly_vertical", type=str, help="Environment mode")   
    parser.add_argument("--train_mode", default="val_unseen", type=str, help="select mode: train, val, test")
    parser.add_argument("--train_stage", default=5, type=int, help="Training stage to evaluate")
...
def evaluate_single_episode(env, policy, device, episode_info, episode_id, args):
    ...
    if args.env_mode == 'dummy_fly_vertical':
        env.init_episode_info(episode_info)
...
def main():
    ...
    if args.env_mode == 'dummy_fly_vertical':
        env = DummyEnv_obstacle_airsimmode_evaluate_fly3D(args)
    else:   
    ...
    for i, episode_info in enumerate(episodes):
        ...
        print(result)
        print(f"Time taken for episode {i}: {time.time() - start_time} seconds, step_count: {result['steps']}, per_step_time: {(time.time() - start_time) / result['steps']} seconds")
