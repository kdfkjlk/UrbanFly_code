...
class Drone_tool():
    ...
    def initialize_client_nomarker(self, client_port):
        print(f"Initializing client on port {client_port}")
    ...
    def set_external_camera_pose(self, obj_position, camera_height=30, help_cam=False):
        ...
        seg_cam = self.client.simGetCameraInfo(camera_name, vehicle_name=self.drone_name, external=True).pose
        ...
        # time.sleep(0.2)
        print('set camera {} at pose {}'.format(camera_name, camera_pose.position, vehicle_name=self.drone_name))
        return camera_pose
    ...
    def get_external_camera_pose(self, help_cam=False):
        if not help_cam:
            ...
            camera_name = self.help_camera_name
        return self.client.simGetCameraInfo(camera_name, vehicle_name=self.drone_name, external=True).pose
    ...
    def check_pose_collision(self, help_cam_position_xy, help_cam_height):
        ...
        for angle in angles:
            ...
            self.client.simSetCameraPose(camera_name, camera_pose, vehicle_name=self.drone_name, external=True)
